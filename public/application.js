...
/***/ function(module, exports, __webpack_require__) {
	...
	var clearFormsOnClose = function clearFormsOnClose() {
	  $('#sign-up')[0].reset();
	  $('#sign-in')[0].reset();
	  $('#change-pw')[0].reset();
	  $('#signUpMessage').text(' ');
...
/***/ function(module, exports, __webpack_require__) {
	...
	var getFormFields = __webpack_require__(22);
	var showStuff = __webpack_require__(23);
	var hideStuff = __webpack_require__(24);
	...
	var onSignOut = function onSignOut(event) {
	  ...
	  api.signOut(data).done(ui.signOutSuccess, showStuff.showThingsOnSignOut, hideStuff.hideThingsOnSignOut).catch(ui.signOutFailure);
	};
	module.exports = {
	  onSignUp: onSignUp,
	  onSignIn: onSignIn,
...
/***/ function(module, exports, __webpack_require__) {
	...
	var $signUpUI = $('#signUpMessage');
	var $signInUI = $('#signInMessage');
	var $changePWUi = $('#changePWMessage');
	...
	var onSignUpError = function onSignUpError(response) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var turnSwitcher = __webpack_require__(19);
	var gameStore = __webpack_require__(13);
	var showStuff = __webpack_require__(20);
	var hideStuff = __webpack_require__(21);
	...
	var createNewGame = function createNewGame() {
	  api.newGame().done(showStuff.showThingsOnSignIn(), hideStuff.hideThingsOnSignIn(), ui.newGameSuccess).catch(ui.newGameFailure);
	};
	...
	var flipPicture = function flipPicture(clickedId, turnCheck) {
	  // figures out what div # clicked what
	  var $clickedCell = $('#' + clickedId);
	  // check the API's game board to see whose turn it is
	  var turn = turnCheck(gameStore.gameStore.cells);
	  // set the new image, should it be x or o
	  // const newSrc = 'http://i.imgur.com/AWv10hk.png'
	  var setImgurSrc = function setImgurSrc(turn) {
	    if (turn === 'o') {
	      return 'https://i.imgur.com/AWv10hk.png';
	    } else {
	      return 'https://i.imgur.com/evjjjNV.png';
	    }
	  };
	  var newSrc = setImgurSrc(turn);
	...
	var onClick = function onClick(div) {
	  api.tttClick(this.id, turnSwitcher.whoseTurn).then(ui.clickSuccess, flipPicture(this.id, turnSwitcher.whoseTurn)).catch(ui.clickFaliure);
	};

	var makeTheBoard = function makeTheBoard() {
	  event.preventDefault();
	  var newSrc = 'https://i.imgur.com/jEDl0rb.png';
	  $('.game-board').children().children().children().attr('src', newSrc);
	  api.newGame().then(ui.newGameSuccess).catch(ui.newGameFailure);
	  $('.ttt-board').on('click', onClick);
	};

	var clearTheBoard = function clearTheBoard() {
	  event.preventDefault();
	  var newSrc = 'https://i.imgur.com/jEDl0rb.png';
	  $('.game-board').children().children().children().attr('src', newSrc);
	};

	var pullGameStats = function pullGameStats() {
	  api.getGames().then(ui.getGamesSuccesss).catch(ui.getGamesFaliure);
	};
	...
	module.exports = {
	  onNewGame: onNewGame,
	  onClick: onClick,
	  makeTheBoard: makeTheBoard,
	  createNewGame: createNewGame,
	  pullGameStats: pullGameStats,
...
/***/ function(module, exports, __webpack_require__) {
	...
	var tttClick = function tttClick(clickedId, turnCheck) {
	  var clickedCell = clickedId * 1;

	  var value = turnCheck(gameStore.gameStore.cells);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var store = __webpack_require__(9);
	var gameStore = __webpack_require__(13);
	var checkGameStatus = __webpack_require__(15);
	var calcGameStats = __webpack_require__(18);
	var $gameStatsDisplay = $('#game-stats-here');
...
/***/ function(module, exports, __webpack_require__) {
	...
	var $gameStatus = $('#game-status');
	var $gameBoard = $('.ttt-board');

	var oWins = function oWins(input) {
	  var result = input.filter(function (e, i, a) {
	    return e === 'o';
	  }).length >= 3;
	  return result;
	};

	var xWins = function xWins(input) {
	  var result = input.filter(function (e, i, a) {
	    return e === 'x';
	  }).length >= 3;
	  return result;
	...
	var cRow = function cRow(input) {
	  var result = input.filter(function (e, i, a) {
	    return e === 'x';
	  }).length >= 1 && input.filter(function (e, i, a) {
	    return e === 'o';
	  }).length >= 1;
	  return result;
	};

	var didSomeOnWin = function didSomeOnWin(data) {
	  var row1 = [data[0], data[1], data[2]];
	  var row2 = [data[3], data[4], data[5]];
	  var row3 = [data[6], data[7], data[8]];
	  var col1 = [data[0], data[3], data[6]];
	  var col2 = [data[1], data[4], data[7]];
	  var col3 = [data[2], data[5], data[8]];
	  var diag1 = [data[0], data[4], data[8]];
	  var diag2 = [data[2], data[4], data[6]];

	  var arraysToCheck = [row1, row2, row3, col1, col2, col3, diag1, diag2];
	  ...
	  if (arraysToCheck.some(xWins)) {
	    ...
	    result = 'x Wins';
	    $gameBoard.off('click');
	    api.endGame().done(ui.endGameSuccess).catch(ui.endGameFailure);
	  }
	...
	var createStatsPredicate = function createStatsPredicate(data) {
	  var row1 = [data[0], data[1], data[2]];
	  var row2 = [data[3], data[4], data[5]];
	  var row3 = [data[6], data[7], data[8]];
	  var col1 = [data[0], data[3], data[6]];
	...
	module.exports = {
	  didSomeOnWin: didSomeOnWin,
	  createStatsPredicate: createStatsPredicate
	};
	/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(2)))
...
/***/ function(module, exports, __webpack_require__) {
	...
	var createGamesArray = function createGamesArray(games) {
	  var result = [];
	...
	var createStatsArray = function createStatsArray(games, pullGameArrays) {
	  var arrayOfGames = pullGameArrays(games);
	  ...
	  var result = 'you played ' + gameResultsArray.length + ' games. Of those you won ' + gameResultsArray.filter(function (e, i, a) {
	    return e === 'x Wins';
	  }).length + ', and lost ' + gameResultsArray.filter(function (e, i, a) {
	    return e === 'o Wins';
	  }).length + ' but did not finish ' + gameResultsArray.filter(function (e, i, a) {
	    return e === 'none';
	  }).length + ' and tied ' + gameResultsArray.filter(function (e, i, a) {
	    return e === 'CATS GAME';
	  }).length;
	...
	module.exports = {
	  ...
	  createStatsArray: createStatsArray
	};
...
/***/ function(module, exports) {
	...
	var whoseTurn = function whoseTurn(gameBoard) {
	  // if there are no x's then it must be x's turn
	  if (gameBoard.filter(function (e, i, a) {
	    return e === 'x';
	  }).length === 0) {
	    return 'x';
	    // if there are the same number of x's and o's in teh array it must be x's turn
	  } else if (gameBoard.filter(function (e, i, a) {
	    return e === 'x';
	  }).length === gameBoard.filter(function (e, i, a) {
	    return e === 'o';
	  }).length) {
	    return 'x';
...
/***/ function(module, exports, __webpack_require__) {
	...
	var $changePwButton = $('.change-pw-button');
	...
	var showThingsOnSignIn = function showThingsOnSignIn() {
	...
	var showThingsOnSignOut = function showThingsOnSignOut() {
	  $signUp.css('visibility', 'visible');
	  $signIn.css('visibility', 'visible');
	};

	module.exports = {
	  showThingsOnSignIn: showThingsOnSignIn,
	  showThingsOnSignOut: showThingsOnSignOut
	};
	/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(2)))
...
/***/ function(module, exports, __webpack_require__) {
	...
	var hideThingsOnSignIn = function hideThingsOnSignIn() {
	  $signUp.css('visibility', 'hidden');
	  $signIn.css('visibility', 'hidden');
	};

	var hideThingsOnSignOut = function hideThingsOnSignOut() {
	...
	module.exports = {
	  hideThingsOnSignIn: hideThingsOnSignIn,
	  hideThingsOnSignOut: hideThingsOnSignOut
	};
	/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(2)))
